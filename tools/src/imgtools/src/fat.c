...
*/
int32_t strrchr_new(char* str, char c) {
    int32_t ret = -1;
...
/**
* @brief Removes trailing white-space characters.
*
...
/**
* @brief Replaces all instances of the passed substring in the passed string. (Make sure that the passed buffer containing the string is big enough.)
*
* @param search the substring to look for.
* @param replace the substring with which to replaces the found substrings.
...
char* stringReplace(const char *search, const char *replace, char *string) {
    ...
    for (;;) {
        ...
        char* tempString = (char*) malloc(strlen(string) - len - searchLen + 1);
        strcpy(tempString, searchStart + searchLen);

        const char* i = replace;
...
FAT* FAT_create(storage_dev_t* storageDev, uint32_t lbaStart, uint32_t lbaLength, uint8_t fileSys) {
    FAT* inst = (FAT*)malloc(sizeof(FAT));
...
uint32_t* FAT_FATgetClusterChain(FAT* inst, uint32_t start_clust, uint32_t start_offset, uint32_t max_size) {
    ...
    uint32_t* clustArr = (uint32_t*) malloc((max_size * 4) / (inst->sectorsPerCluster * 512) + 4);
    //listHead_t* clustList = listCreate();
    uint32_t i = 0;
...
uint32_t* FAT_readFileEntryLocation(FAT* inst, char* filepath) {
    ...
    //char tmp_filepath[tmp_filepath_len]; !!! MEMORY LEAK !!!
    char* tmp_filepath = (char*) malloc(tmp_filepath_len);
    ...
    for (uint32_t k = 0; k < path_size; ++k) {
        ...
        int32_t lastDot = strrchr_new(spl_path[k], '.');
        if (lastDot == -1) lastDot = cur_file_len;
        //char cur_file_name[lastDot + 1]; !!! MEMORY LEAK !!!
        char* cur_file_name = (char*) malloc(lastDot + 1);
        for (uint32_t j = 0; j < lastDot; ++j) {
            cur_file_name[j] = spl_path[k][j];
        }
        cur_file_name[lastDot] = 0; //Null termination
        // char cur_file_ext[cur_file_len - lastDot]; !!! MEMORY LEAK !!!
        char* cur_file_ext = (char*) malloc(cur_file_len - lastDot);
            ...
            for (uint32_t j = 0; j < inst->maxRootDirEntries / 16; ++j) {
                ...
                for (uint32_t i = 0; i < 512; i += 32) {
                    ...
                    if (long_fname_buffer[0] == 0) { //if file name is short
                        ...
                        lastDot = strrchr_new(long_fname_buffer, '.');
                        if (lastDot == -1) lastDot = cur_file_len;
                    ...
                    if (strcmp(cur_filename, cur_file_name) == 0) { //if the requested file name matches the file name of the current entry
                        ...
                        if (k == (path_size - 1)) { //if is final file
                            uint32_t* entry_loc = (uint32_t*) malloc(16);
                            entry_loc[0] = j;
                            entry_loc[1] = i;
                            if (ret_has_long) {
...
uint8_t FAT_createDirectory(FAT* inst, char* filepath) {
    ...
    //char path_container[path_container_len]; !!! MEMORY LEAK !!!
    char* path_container = (char*) malloc(path_container_len);
...
char* FAT_readVolumeLabel(FAT* inst) {
    if (inst->fileSys <= FAT16) {
        for (uint16_t j = 0; j < inst->maxRootDirEntries / 16; ++j) {
            uint8_t cur_sec_bytes[512];
            storage_readSector(inst->storageDev, inst->lbaStart + inst->reservedSectors + inst->sectorsPerFAT*inst->FATCopies + j, cur_sec_bytes, 512);
...
void FAT_readFileContents(FAT* inst, uint8_t* entry, uint8_t* buffer, uint32_t start, uint32_t len) {
    ...
    //uint8_t cur_sec_bytes[512 * inst->sectorsPerCluster]; !!!MEMORY LEAK !!!
    uint8_t* cur_sec_bytes = (uint8_t*)malloc(512 * inst->sectorsPerCluster);
    uint32_t pos = (start / (inst->sectorsPerCluster * 512))*(inst->sectorsPerCluster * 512);
    if (clustArr[0] == 0) cur_clust = 0;
...
char* FAT_longNameToShortName(FAT* inst, char* filepath) {
    ...
    int32_t lastDot = strrchr_new(file_str, '.');
    if (lastDot == -1) lastDot = file_str_len;
    //char filename[lastDot + 1]; !!!MEMORY LEAK !!!
    char* filename = (char*)malloc(lastDot + 1);
    for (uint32_t j = 0; j < lastDot; ++j) {
        filename[j] = file_str[j];
    }
    filename[lastDot] = 0; //Null termination
    //char fileext[file_str_len - lastDot]; !!!MEMORY LEAK !!!
    char* fileext = (char*)malloc(file_str_len - lastDot);
...
uint8_t* FAT_createFile(FAT* inst, char* filepath, uint8_t attribute) {
    ...
    //Get the file container path
    //char spl_path_container[path_size - 1][FAT_MAX_FNAME_LEN]; !!! MEMORY LEAK !!!
    char spl_path_container[FAT_MAX_PATH_FILES][FAT_MAX_FNAME_LEN];
    for (uint32_t k = 0; k < (path_size - 1); ++k) {
        for (uint32_t l = 0; l < FAT_MAX_FNAME_LEN; ++l) {
    ...
    int32_t lastDot = strrchr_new(new_file_str, '.');
    if (lastDot == -1) lastDot = cur_file_len;
    //char new_filename[lastDot + 1]; !!!MEMORY LEAK !!!
    char* new_filename = (char*)malloc(lastDot + 1);
    for (uint32_t j = 0; j < lastDot; ++j) {
        new_filename[j] = new_file_str[j];
    }
    new_filename[lastDot] = 0; //Null termination
    //char new_fileext[cur_file_len - lastDot]; //TODO: I think there has be a +1 !!! MEMORY LEAK !!!
    char* new_fileext = (char*)malloc(cur_file_len - lastDot);
    for (uint32_t j = (lastDot + 1); j < cur_file_len; ++j) {
    ...
    entry = (uint8_t*) malloc(32);//entry = (uint8_t [32]){0};
    if ((strlen(new_filename) > 8) || (strlen(new_fileext) > 3)) { //if long filename
        ...
        long_fname_entry_buffer = (uint8_t*) malloc(long_fname_entry_count * 32);
...
uint8_t FAT_readFileEntryByIndex(FAT* inst, file_t* file_inst, char* filepath, uint32_t index) {
    ...
    for (uint32_t k = 0; k < path_size + 1; ++k) {
        ...
        if (k < path_size) {
            ...
            lastDot = strrchr_new(spl_path[k], '.');
            if (lastDot == -1) lastDot = cur_file_len;
        }
        //char cur_file_name[lastDot + 1]; !!!MEMORY LEAK !!!
        char* cur_file_name = (char*)malloc(lastDot + 1);
